...
import { verifyPaystackForReference } from "@/lib/server/payment-service";
...
export async function POST(request: Request) {
  const limited = rateLimit({
    key: clientKeyFromRequest(request, "paystack-verify"),
    limit: 30,
    windowMs: 60_000,
  });
  if (!limited.ok) {
  ...
  try {
    ...
    const result = await verifyPaystackForReference(parsed.data.reference);
    if (!result.ok) {
      reportPaymentFailure("Paystack verify rejected", {
    ...
    reportPaymentFailure("Paystack verify failed", {
